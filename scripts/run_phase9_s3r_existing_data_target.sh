...
scratch="/home/avis/rvt-data/audit/phase9g-a1s3r-existing-data-retry-v1"
...
docker run --rm \
  ...
  -w /opt/rvt \
  "$image" \
  python /diagnostic/audit_phase9_s3r_existing_data.py \
    --root /opt/rvt \
    --checkpoint /diagnostic/phase9_s3_staging_checkpoint_v1.json \
    --previous-dependency /diagnostic/phase9_s3_staging_dependency_audit_v1.json \
    --execution-environment QUALIFIED_PRODUCTION_DOCKER_LINUX_X86_64 \
    --output /out/phase9_s3_existing_data_requalification_docker_v1.json
