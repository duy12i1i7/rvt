...
scratch="/home/avis/rvt-data/audit/phase9g-a1s3-reference-diagnostics"
...
docker run "${common[@]}" "$image" \
  python /diagnostic/diagnose_phase9_s3_width.py \
  --root /opt/rvt \
  --execution-environment QUALIFIED_PRODUCTION_DOCKER_LINUX_X86_64 \
  --output /out/phase9_s3_width_derivation_docker_v1.json

docker run "${common[@]}" "$image" \
  python /diagnostic/audit_phase9_s3_population.py \
  --root /opt/rvt \
  --execution-environment QUALIFIED_PRODUCTION_DOCKER_LINUX_X86_64 \
  --output /out/phase9_s3_population_audit_docker_v1.json
