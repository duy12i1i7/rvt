...
\node[feat] (pos)   at (0, 0.0) {$\bfp_i, \bfv_i, \mathrm{head}_i$};
\node[feat] (goal)  at (0,-0.9) {$\mathbf{g}_i, \hat{\mathbf{g}}_i$};
\node[feat] (obs)   at (0,-1.8) {obs./corridor feat.};
\node[feat] (lidar) at (0,-2.7) {LiDAR$_{36}$};
\node[feat] (topo)  at (0,-3.6) {$\mathrm{onehot}(m_t)$};

\node[note, above=0.15cm of pos] {Derived node features};
...
\node[block, fill=green!10, minimum height=1.6cm,
      minimum width=2.0cm]
    (gat) at (6.2,-2.6) {Graph Attn\\Layer $\times 3$\\{\tiny attn + msg pass}};
...
\node[score] (unc) at (9.5,-2.7)
    {Uncertainty\\$\hat{\bm{\sigma}} \in \mathbb{R}^5$};
% Consensus
\node[head] (cons) at (9.5,-4.0)
    {Consensus\\$\bm{\ell} \in \mathbb{R}^5$};

% grad scale annotation
\node[note] at (7.9, -1.0) {\textcolor{red!60}{grad\_scale($1/(K+1)$)}};
...
\node[score, fill=purple!15, minimum width=2.3cm]
    (rfn_out) at (12.8,-1.5)
    {$\tilde{r}_\tau = \hat{r}_\tau - \operatorname{std}(\hat{\mathbf{r}})\hat{\sigma}_{\tau}$};
