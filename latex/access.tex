...
\title{Rollout-Supervised Resilient Formation Reconfiguration in Multi-Robot Systems}
...
\markboth
{Nguyen \headeretal: Rollout-Supervised Resilient Formation Reconfiguration in Multi-Robot Systems}
{Nguyen \headeretal: Rollout-Supervised Resilient Formation Reconfiguration in Multi-Robot Systems}
...
\begin{abstract}
Decentralized multi-robot navigation in cluttered environments has several important challenges, including collision avoidance and formation recovery. This paper proposes a rollout-supervised and recoverability-driven framework for resilient formation reconfiguration in multi-robot systems. First, the local interactions among robots are represented as a graph using navigation and formation features. Second, different reconfiguration modes are evaluated by using mode-conditioned control commands and recoverability scores. This allows the system to compare different choices, such as keeping the formation, moving in a line, or splitting the formation. Third, the learned recoverability scores are used for online formation-mode selection, latent geometry adaptation, and risk-triggered safety projection. Short-horizon rollout supervision helps the model select modes that support safe motion and recovery after formation deformation. Experiments with 2 to 24 robots in 2D cluttered environments show that the proposed framework improves overall performance compared with the internal baselines, especially in success rate, collision-free navigation, formation satisfaction, and collapse reduction.

\end{abstract}

\begin{keywords}
Multi-robot systems, formation control, graph neural networks, and autonomous navigation.
\end{keywords}
...
optional projection safety filter intervenes only when risk is high or when
...
and a risk-triggered projection safety filter.
...
& \parbox[c]{0.08\textwidth}{\centering\textbf{Progress}\\\textbf{safety}\\\textbf{filter}} \\
\midrule
...
\Cref{fig:rfn} summarizes the model-level dataflow used to approximate
this decision rule, while \Cref{fig:architecture} later shows how the
trained model outputs are consumed by the robot-side runtime.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/main/f2.pdf}
\caption{Model-level processing pipeline of RVT-Swarm. Local robot observations and neighboring-agent information are transformed into graph tensors, encoded by attention-based message passing, and decoded into the decision quantities required by the controller: topology preference, recoverability, uncertainty, auxiliary context, and topology-conditioned actions.}
\label{fig:rfn}
\end{figure*}
...
Field Network (RFN) shown in \Cref{fig:rfn}. We then describe the
counterfactual rollout supervision used to train it, and finally present
the recoverability-aware execution stack in \Cref{fig:architecture} that
uses the trained outputs online.

\subsection{Local Graph Encoder}
\label{sec:encoder}

At each step, the swarm builds a fixed-$K$ nearest-neighbor interaction
graph from the robot-side policy observation. As summarized in the input
and graph-preparation columns of \Cref{fig:rfn}, the adapter starts from
local state, LiDAR-style sensing, the shared goal, the current topology
runtime state, and nearby peer messages. These quantities are packed into
node features, edge features, an edge index, a batch index, and a pooled
raw graph context before they enter the neural encoder.

We use $o_k$ to denote the raw team-level observation returned by the
simulator at step $k$, and $(\mathbf{x}_i,\mathbf{e}_{ij})$ to denote
the derived node and edge feature tensors constructed from $o_k$ before
they are passed to the graph encoder. Thus $o_k$ is the observation
object, whereas $\mathbf{x}_i$ is the per-robot feature vector extracted
from it. In the main text, only the feature families matter: node tensors
combine kinematics, goal cues, formation context, obstacle and corridor
cues, global topology state, local safety indicators, and LiDAR-style
sensing, while edge tensors encode relative geometry, relative motion,
spacing, and TTC-style safety context. Because the model equations depend
on these feature families rather than on a particular ordering of
bookkeeping terms, the encoder discussion here focuses on the inductive
bias imposed by the graph representation.
...
separation is deliberate and matches the right side of \Cref{fig:rfn}.
The continuous action is local and per-robot, whereas topology,
uncertainty, auxiliary context, and recoverability are graph-level
...
$x_k$. The quantities
$(\mathbf{U}_i,\tilde{\mathbf{r}},\bm{\ell},\hat{\mathbf{a}},
\hat{\bm{\sigma}})$ are the model outputs shown on the right of
\Cref{fig:rfn}. At runtime, the selector consumes the graph-level outputs
to choose a structural topology $\tau^\star$. The controller then slices
the action bank to obtain $\bfu_i=\mathbf{U}_i[\tau^\star]$, and the
safety filter modifies this nominal action only if geometric risk is high or
...
whereas the graph-level branches are driven from detached pooled latents
and raw pooled inputs. This keeps the action representation anchored
while still exposing topology and recoverability structure to the
network. With these outputs in place, we next explain how counterfactual
rollouts supervise the RFN branches before returning to the online
execution stack that uses the trained outputs at runtime.
...
conservative soft topology distribution, the selected-topology anchor, the full topology-conditioned action bank, the \textsc{keep} anchor, and the graph-level auxiliary target shown in \Cref{fig:rfn}.
...
configuration shared across all reported runs. Best-checkpoint selection
is based on a periodic rollout-validation score,
\begin{equation}
...
actually completes the task, and keeps the emphasis on target
construction and objective design rather than minibatch optimization
mechanics.
...
\caption{End-to-end robot execution flow of RVT-Swarm. The ROS~2 interface collects onboard sensing and communication data, the policy module evaluates formation-topology alternatives, and the runtime control layer selects a recoverable topology, updates the formation state, applies risk-triggered safety projection, and sends velocity commands to the robot base.}
...
a safety filter only when geometric risk or globally negative recoverability
justifies intervention. \Cref{fig:architecture} summarizes this full
robot-side inference and command-execution flow. 
\begin{comment}
We present its three runtime pieces in the order
...
margins from the deployment path. The topology-selection rule is applied
before the risk-triggered projection safety filter described below.
...
does not count pure scale oscillations as topology churn.

\subsubsection{Risk-Triggered Safety Projection}
\label{sec:safety_filter}

After topology and desired geometry have been updated, the nominal
action is passed through a final safety layer. The safety filter activates
...
half-planes and the acceleration ball. The final safety-filtered action is
...
makes the safety filter more willing to intervene during unrecoverable crises.
...
\label{eq:safety_filter_scales}
\end{equation}
Here $\rho_{\mathrm{th}}$ is the risk trigger, and $\beta_{\max}$ is
the maximum blend cap. The safety filter uses these two scales to blend the
...
latent geometry update, and the risk-triggered safety filter define one online
control step.
\begin{comment}
summarized in Appendix~\Cref{alg:online_pipeline}.
\end{comment}
...
read as success rates rather than binary outcomes. Deadlock,
irreversible collapse, and recoverability calibration are computed by
the same evaluation script for every method.
...
\textbf{GNN} uses the shared graph backbone with only an action
...
a CBF-QP reactive filter, ORCA avoidance, and a centralized
MPC-inspired proxy. For code-level reproducibility, these historical
baselines are instantiated as follows:
\begin{itemize}
    \item \textbf{ORCA} uses clipped goal tracking with velocity damping plus pairwise and obstacle repulsion, with no topology adaptation.
    ...
    \item \textbf{CBF-QP} starts from the \textsc{keep}-topology expert action and adds stronger reactive repulsion terms before clipping.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/results/all_trajectories.png}
\caption{Qualitative trajectory comparison across team sizes and controllers. Columns correspond to team sizes $N\in\{2,4,\ldots,24\}$, and rows correspond to the compared controllers. Colored curves denote individual robot trajectories, red disks denote obstacles, green disks show the goal, and black markers denote terminal robot positions. The figure complements the main-results table by showing how the aggregate success, collision-free, and formation scores appear as spatial trajectory patterns in the cluttered benchmark.}
...
and collapse rate ($0.472$ vs.\ $0.430$). CBF-QP remains a strong
...
($0.469$ vs.\ $0.430$). GNN is the closest learned baseline in
...
\Cref{fig:all_trajectories} supports the aggregate trends in
\Cref{tab:main_results}. RVT-Swarm tends to preserve a more coherent
flow through the obstacle cluster as $N$ grows, whereas weaker baselines
more often produce tangled paths, terminal clustering near the
bottleneck, or large detours. This qualitative behavior explains why
the full method improves the combined success criterion rather than only
one isolated metric.
...
$-$ Progress safety filter & 0.312 & 0.486 & 0.438 & \textbf{0.028} & 0.468 & \textbf{0.116} \\
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/results/ablation_trajectories.png}
\caption{Qualitative trajectory ablation of RVT-Swarm. Columns correspond to team sizes $N\in\{2,4,\ldots,24\}$, and rows correspond to the full model and ablated variants. Colored curves show individual robot trajectories, red disks show obstacles, green disks show the goal, and black markers show terminal robot positions. The visualization helps interpret the ablation table by exposing how removing recoverability, counterfactual supervision, topology adaptation, formation-scale adaptation, or the progress safety filter changes the spatial organization of the team.}
...
\emph{$-$ Progress safety filter} remains very close to the full model on
...
effects of the progress safety filter and explicit topology adaptation appear
more mixed at the aggregate level, even though removing topology still
worsens several safety-related metrics.

The ablation table identifies which components matter quantitatively,
but the scalar metrics alone do not fully explain how the degraded
variants behave. \Cref{fig:ablation_trajectories} therefore visualizes
matched rollout trajectories for the full model and each ablated
variant. The purpose is diagnostic: the figure links the drops in
success, formation satisfaction, collapse, and topology switching in
\Cref{tab:ablation} to spatial effects in the team motion.

The qualitative ablation is consistent with the metric trends in
\Cref{tab:ablation}. Removing recoverability produces less organized
motion and more terminal congestion near obstacles, matching the large
drop in success and formation satisfaction. Removing counterfactual
supervision and fixing the formation scale also lead to less stable
trajectory organization, especially for larger teams. By contrast, the
progress-safety-filter and topology-only ablations remain closer to the full
model in aggregate, which is consistent with their smaller metric gaps.
...
lexicographic rule, while the optional safety filter uses geometry-normalized
intervention scales.
...
\ENSURE Selected topology $\tau^\star$, safety-filtered action
...
risk-triggered projection safety filter to obtain $\bfu^{\mathrm{safe}}$
...
$m \in \{\textsc{gnn}, \textsc{instant\_cert},
\textsc{rvt\_swarm}\}$
...
\begin{itemize}
    ...
    topology-state updates, counterfactual selector, and optional safety filter
    ...
    \{\textsc{gnn}, \textsc{instant\_cert}, \textsc{rvt\_swarm}\},
    evaluation of those three learned methods plus
    \{\textsc{adaptive\_formation}, \textsc{cbf\_qp},
    \textsc{orca}, \textsc{centralized\_mpc}\}, shared ablation
    dataset generation, six ablation train/eval runs, and optional GIF
    visualization.
    \item The ablation study evaluates six toggles: Full, No
    Recoverability, No Counterfactual, No Progress Safety Filter, No Topology,
